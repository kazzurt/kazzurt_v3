...
img = Image.open("/home/pi/kz3/ssbd.jpg") #load in da image ya
img = img.rotate(90)
...
#             if (p[2,i]) < 150:
#                 p[2,i]*= .5*np.sin(cnt/20)+.5
            if (p[2,i]) < 100: # 100*np.sin(cnt/10)+150: p[0,i]+p[1,i]+
                p[:,i] = (.25*np.sin(cnt/4)+.5)*p2[:,i]
