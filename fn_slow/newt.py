...
class newt:
    ...
    def newt(y):
        ...
        rtim +=1/y
        n1 = np.linspace(0,1300-1,10).astype(int)
        ...
        p[2, n1]    = (.5*np.sin(rtim/3+2*np.pi/3*(np.sin(rtim/20)+np.pi/3))+.5)*255

        
        n = 1
        p[:, n:] = p[:, :-n]#**1.01
        if rtim%10 == 0:
          p[:, rn.randint(0,1300-1,size=rn.randint(0,int(20*np.sin(rtim/3)+25)))]   = 0
