...
#                         rtim4 = 0
            ...
            for i in arx:
                red_ar[i,ary] =  (.5*np.sin(rtim11/4 + ary/(.5*yf)  + i/(.2*xf)             ) +.5)*255
                gre_ar[i,ary] =  (.5*np.sin(rtim11/4 + ary/(.75*yf) + i/(.3*xf) + 5*np.pi/3) +.5)*255
                blu_ar[i,ary] =  (.5*np.sin(rtim11/4 + ary/(.1*yf)  + i/(.2*xf) + 2*np.pi/3 ) +.5)*255
#             if cy11>1:
...
#                             bdir=-1 #this will make us start stepping cy backwards
            kk = .5*np.sin(rtim11/5)+5
            p[0,:] = (.5*np.sin(rtim11/20/4)+.5)          *coo11[0]*quadratize.flatMatQuads(red_ar)
            p[1,:] = (.5*np.sin(rtim11/20/4+kk*np.pi/3)+.5)*coo11[1]*quadratize.flatMatQuads(gre_ar)
            p[2,:] = (.5*np.sin(rtim11/20/4+7*np.pi/3)+.5)*coo11[2]*quadratize.flatMatQuads(blu_ar)
