...
class tetris2:
    def tetris2(y):
        ...
        if cnt3t>6:
            ...
            evens = np.linspace(0,pix,rn.randint(5,25)).astype(int)
            cnt3t = 0
        if cnt1t>20 and m2<tethresh:# and cnt3>5:
            p[:,odds] = 0
            odds = np.linspace(0,pix,rn.randint(5,25)).astype(int)
            for i in np.linspace(0,len(clr[:,0])-1,len(clr[:,0])).astype(int):
                p[0,evens[(i)*div:(i+1)*div]] = clr[i,0]
                p[1,evens[(i)*div:(i+1)*div]] = clr[i,2]
                p[2,evens[(i)*div:(i+1)*div]] = clr[i,1]
            cnt3t = 0
        if cnt3t>25:
            tethresh *=.95
