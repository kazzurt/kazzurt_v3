...
class life_random:
    def life_random(y):
        ...
        old_pairs,all_pairs,gen_counter,old_gen_x_random,old_gen_y_random, phase_offran, gc2
        
        #time.sleep(0.02)
        
        arby_life = np.zeros((40,25))
        gc2+=1
        if gen_counter == 0 or gc2 == 1:
            for cur_ix in range(len(old_gen_x_random)):
                cur_pair = (old_gen_x_random[cur_ix],old_gen_y_random[cur_ix])
        ...
        red_val = .5*np.sin(np.pi*gen_counter/15)+.5
        green_val = .5*np.sin(np.pi*gen_counter/15+np.pi/3 + phase_offran)+.5
        blue_val = .5*np.sin(np.pi*gen_counter/15+2*np.pi/3+2*phase_offran)+.5
