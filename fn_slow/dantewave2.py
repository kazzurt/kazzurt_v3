...
class dantewave2:
    def dantewave2(y): #function 51
        ...
        phas = .5*np.sin(rtim/3)+.5
        for i in ary:
            if i%2==0:
                red_ar[arx,i] =  fac*(.5*np.cos(-rtim/4 + arx/yf + i/xf            +np.pi/7)+.5)
                gre_ar[arx,i] =  fac*(.5*np.cos(-rtim/4 + arx/yf + i/xf + 5*np.pi/3+phas+np.pi/7)+.5)
                blu_ar[arx,i] =  fac*(.5*np.cos(-rtim/4 + arx/yf + i/xf + 7*np.pi/3+phas+np.pi/7)+.5)
#             else:
#                 red_ar[arx,i] =  (1-fac)*(.5*np.cos(rtim/4 + arx/yf + i**(.5+rtim/10)/xf            )+.5)
#                 gre_ar[arx,i] =  (1-fac)*(.5*np.cos(rtim/4 + arx/yf + i**(.5+rtim/10)/xf + 2*np.pi/3)+.5)
#                 blu_ar[arx,i] =  (1-fac)*(.5*np.cos(rtim/4 + arx/yf + i**(.5+rtim/10)/xf + 4*np.pi/3)+.5)
        for i in arx:
            if i%2==1:
                red_ar[i,ary] =  (1-fac)*(.5*np.sin(-rtim/4 + ary/xf + i/yf            +np.pi/4*phas)+.5)
                gre_ar[i,ary] =  (1-fac)*(.5*np.sin(-rtim/4 + ary/xf + i/yf + 2*np.pi/3*phas)+.5)
                blu_ar[i,ary] =  (1-fac)*(.5*np.sin(-rtim/4 + ary/xf + i/yf + 4*np.pi/3*phas)+.5)
#             else:
