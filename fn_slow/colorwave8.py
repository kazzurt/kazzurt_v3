...
class colorwave8:
    ...
    def colorwave8(y):
            ...
            xf = x_off11 + num
            yf = y_off11 - num
            n = 2*np.sin(rtim11/5)+20
            for i in arx:
                blu_ar[i,ary] =  (.5*np.sin(rtim11/4 + (ary/(.1*yf)+n) + i/(.2*xf)            )+.5)*255
                red_ar[i,ary] =  (.5*np.sin(rtim11/4 + (ary/(.2*yf)+n) + i/(.3*xf) + 2*np.pi/3)+.5)*255
                gre_ar[i,ary] =  (.5*np.sin(rtim11/4 + (ary/(.3*yf)+n) + i/(.4*xf) + 5*np.pi/3)+.5)*255
