...
class colorwave4:
    def colorwave4(y): #function 51
        ...
        yf = (2*np.sin(rtim/15)+(np.sin(rtim/15)+3.75))
        xf = (2*np.cos(rtim/15)+(np.cos(rtim/15)+3.75))
        
        for i in arx:
            if i>config.ARX//2 and i%2==1:
                red_ar[i,ary] =  (.5*np.cos(-rtim/4 + ary/yf + i/xf            +np.pi/4)+.5)
                gre_ar[i,ary] =  (.5*np.cos(-rtim/4 + ary/yf + i/xf + 2*np.pi/3+phas+np.pi/4)+.5)
                blu_ar[i,ary] =  (.5*np.cos(-rtim/4 + ary/yf + i/xf + 4*np.pi/3+phas+np.pi/4)+.5)
#             else:
...
#                 blu_ar2[i,ary] = 0
        ...
        p[0,:] = (quadratize.flatMatQuads(red_ar)/2+quadratize.flatMatQuads(red_ar2)/2) * 255
        p[1,:] = (quadratize.flatMatQuads(gre_ar)/2+quadratize.flatMatQuads(gre_ar2)/2) * 255
        p[2,:] = (quadratize.flatMatQuads(blu_ar)/2+quadratize.flatMatQuads(blu_ar2)/2) * 255
