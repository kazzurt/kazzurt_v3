...
class colorwave9:
    ...
    def colorwave9(y):
            ...
            xf = x_off11 + num
            yf = y_off11 - num
            n = 2*np.sin(rtim11/5)+40
            for i in arx:
                blu_ar[i,ary] =  (.5*np.sin(rtim11/(n/4) + (ary/(.1*yf)+n) + i/(.2*xf)            )+.5)*255
                red_ar[i,ary] =  (.5*np.sin(rtim11/(n/4) + (ary/(.2*yf)+n) + i/(.3*xf) + 2*np.pi/3)+.5)*255
                gre_ar[i,ary] =  (.5*np.sin(rtim11/(n/4) + (ary/(.3*yf)+n) + i/(.4*xf) + 5*np.pi/3)+.5)*255
            
#             blu_ar = rotatee(blu_ar, rtim31, 2)
#             gre_ar = rotatee(gre_ar, rtim31, 2)
#             red_ar = rotatee(red_ar, rtim31, 2)
            
            p[0,:] = (.5*np.sin(rtim11/n/4)+.5)          *coo11[0]*quadratize.flatMatQuads(red_ar)
            p[1,:] = (.5*np.sin(rtim11/n/4+(n**.5)*np.pi/3)+.5)*coo11[1]*quadratize.flatMatQuads(gre_ar)
            p[2,:] = (.5*np.sin(rtim11/n/4+n*np.pi/3)+.5)*coo11[2]*quadratize.flatMatQuads(blu_ar)
            #WHAT DAFUQ this is awesome how did I do this
            return p
