...
#         self.mainloop = 
    ...
    def inc(y):
        ...
        fg = 1
        #print(cnt)
        upp = int(2*np.sin(cnt/15)+4)
        dow = int(4*np.cos(cnt/15)+7)
        if fg>thresh_inc:
            
            p[0,np.random.randint(len(p[0,:]),size=upp)] = rn.randint(0,255)
            p[1,np.random.randint(len(p[0,:]),size=upp)] = rn.randint(0,255)
            p[2,np.random.randint(len(p[0,:]),size=upp)] = rn.randint(0,255)
            cr+=1
            hg = 0
            hg2 +=1
        #print(cnt)
