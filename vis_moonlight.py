...
from fn_slow.colorwave2 import colorwave2
from fn_slow.colorwave3 import colorwave3
...
from fn_slow.colorwave7 import colorwave7
from fn_slow.colorwave8 import colorwave8
from fn_slow.colorwave9 import colorwave9
from fn_slow.colorwave09 import colorwave09
from fn_slow.dantewave import dantewave
from fn_slow.dantewave2 import dantewave2
...
from fn_slow.umbrella import umbrella1
from fn_slow.umbrella_dark import umbrella_dark
from fn_slow.insta import insta
from fn_slow.fract1 import fract1
from fn_slow.subs1 import subs1
from fn_slow.pointwave import pointwave
from fn_slow.colorscroll import colorscroll
from fn_slow.vis_incs import incs
from fn_slow.heart1 import heart1
from fn_slow.ddr2 import ddr2
from fn_slow.peace import peace
from fn_slow.smiley import smiley
from fn_slow.pot import pot
from fn_slow.alien import alien
from fn_slow.poop import poop
from fn_slow.ssbd import ssbd
from fn_slow.newt import newt
from fn_slow.newt2 import newt2
import viz_mf
import quadratize
import kzbutfun
...
button = [Button(17),Button(27),Button(22),Button(18)]


functs = ["ssbd","colorwave01","colorwave02","colorwave4","colorwave25","colorwave26","colorwave6","colorwave7","bessel1","bessel2","bessel3","bessel4", "pointwave", \
          "colorwave8","colorwave9","colorwave09","heart1","peace","breathe", "radial_wave2","radial_wave4",\
          "sweetscroll","sweetscroll2","scroll4","radial_wave5","inc","ddr2","umbrella", "umbrella_dark","insta","colorwave1",\
          "smiley","poop","newt","newt2", "dantewave","dantewave2", "sweetscroll","subs1","alien"] #"radial_wave6" "sweetscroll","scroll4","radial_wave5"
...
ling   = True
bltyp  = 1
umb_init = rn.randint(0,len(quadratize.funktions)-1)
umb_init2 = rn.randint(0,len(quadratize.funktions)-1)
...
def visualize_kurt(y):
    global p, loop, a, bb, fun, ct, s, kz3, mp, m2, m3, m4, p2, bby, lst_time, fun_cut, transc, trs, tran, last_fun, csp, \
           ...
           trcn, ling, bltyp, butc, umb_init, umb_init2, sto, sto2,cntlo, z, but1, brit
    #z=5
    cntlo+=1
    ...
    if button[0].is_pressed:
        ...
        #but1 = 1
        time.sleep(1)
        button[0].wait_for_press()
        print("play")
        time.sleep(1)
    else:
        ct+=1
        
#     if button.is_pressed and but1 >=2:
#         print("play")
#         but1 = 0
    ...
    if ct >= t11 and ct < t12:       #first half of linger
        ...
        FF = functs[funs[ft]]
        FF2 = functs[funs[ft+1]]
        if FF != "umbrella" and FF != "umbrella_dark" and FF != "fract1" and FF != "ddr2" and FF != "poop" and FF != "pot" \
           and FF != "insta" and FF != "heart1" and FF != "smiley":
            if FF2 != "umbrella" and FF2 != "umbrella_dark" and FF2 != "fract1" and FF2 != "ddr2" and FF2 != "poop" and FF2 != "pot" \
           and FF2 != "insta" and FF2 != "heart1" and FF2 != "smiley":
                umb_init = rn.randint(0,len(quadratize.funktions)-1)
                umb_init2 = rn.randint(0,len(quadratize.funktions)-1)
                print("UMB RAND")
...
#         t15 = 120
        ...
        ni = 0#1#20*np.sin(cntlo/200)+20
        #t11 = rn.randint(50,70)
        
        t12 = t11 + rn.randint(10,20)
        t13 = t12 + rn.randint(10,20)
        t14 = t13 + rn.randint(10,20)
        t15 = t14 + 200#rn.randint(100,70)
        ct = t11
        print('t11',t11)
        print('t12',t12)
        print('t13',t13)
        ...
        print('============New timers=======================New Function: ', functs[funs[ft]])
...
def visualize_colorwave25(y):
    ...
    return p
def visualize_colorwave4(y):
    global p
    p = colorwave4.colorwave4(y)
    return p
def visualize_dantewave(y):
    global p
    p = dantewave.dantewave(y)
    return p
def visualize_dantewave2(y):
    global p
    p = dantewave2.dantewave2(y)
    return p
...
def visualize_colorwave7(y):
    ...
    return p
def visualize_colorwave8(y):
    global p
    p = colorwave8.colorwave8(y)
    return p
def visualize_colorwave9(y):
    global p
    p = colorwave9.colorwave9(y)
    return p
def visualize_colorwave09(y):
    global p
    p = colorwave09.colorwave09(y)
    return p
...
def visualize_colorscroll(y):
    ...
    return p
def visualize_alien(y):
    global p
    p = alien.alien(y, umb_init)
    return p
#########################################################################
...
def visualize_sweetscroll(y):
    ...
    return p
def visualize_sweetscroll2(y):
    global p
    p = scroll.sweetscroll2(y)
    return p
...
def visualize_heart1(y):
    ...
    return p
def visualize_smiley(y):
    p = smiley.smiley(y,umb_init)
    return p
def visualize_pot(y):
    p = pot.pot(y,umb_init)
    return p
def visualize_ddr2(y):
    p = ddr2.ddr2(y,umb_init)
    return p
def visualize_peace(y):
    p = peace.peace(y,umb_init, umb_init2)
    return p
def visualize_poop(y):
    p = poop.poop(y,umb_init)
    return p
def visualize_ssbd(y):
    p = ssbd.ssbd(y,umb_init)
    return p
...
def microphone_update(audio_samples):
    global y_roll, prev_rms, prev_exp, prev_fps_update, pq, p, slow, mel, mel2
...
if sys.argv[1] == "spectrum":
...
elif sys.argv[1] == "sweetscroll2":
        visualization_type = visualize_sweetscroll2
...
elif sys.argv[1] == "colorwave4":
        visualization_type = visualize_colorwave4
...
elif sys.argv[1] == "colorwave8":
        visualization_type = visualize_colorwave8
elif sys.argv[1] == "colorwave9":
        visualization_type = visualize_colorwave9
elif sys.argv[1] == "colorwave09":
        visualization_type = visualize_colorwave09
elif sys.argv[1] == "dantewave":
        visualization_type = visualize_dantewave
elif sys.argv[1] == "dantewave2":
        visualization_type = visualize_dantewave2
...
elif sys.argv[1] == 'smiley':
        visualization_type = visualize_smiley
elif sys.argv[1] == 'alien':
        visualization_type = visualize_alien
elif sys.argv[1] == 'ddr2':
        visualization_type = visualize_ddr2
        ...
        visualization_type = visualize_pot
elif sys.argv[1] == 'poop':
        visualization_type = visualize_poop
elif sys.argv[1] == 'ssbd':
        visualization_type = visualize_ssbd
