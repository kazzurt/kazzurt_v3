...
arx         = np.linspace(0,config.ARX-2,config.ARX//2).astype(int)
arx2        = np.linspace(1,config.ARX-1,config.ARX//2).astype(int)
...
class pointwave:
    def pointwave(y):  
        ...
        for i in arx:
            ...
            gre_ar[i,ary] =  (.5*np.sin(rtim/6 + ary/ydi + i/xdi + 2*np.pi/3+phas)+.4)
            blu_ar[i,ary] =  (.5*np.sin(rtim/8 + ary/ydi + i/xdi + 4*np.pi/3+2*phas)+.4)        
        for i in arx2:    
            gre_ar2[i,ary] =  (.5*np.sin(-rtim/4 - (ary+1)/ydi + i/xdi            )+.4)
            blu_ar2[i,ary] =  (.5*np.sin(-rtim/6 - (ary+1)/ydi + i/xdi - 2*np.pi/3+phas)+.4)
            red_ar2[i,ary] =  (.5*np.sin(-rtim/8 - (ary+1)/ydi + i/xdi - 4*np.pi/3+2*phas)+.4)  
#         for i in arx:
...
#             blu_ar2[i,ary] =  (.5*np.sin(rtim/sec + ary/ydi + i/xdi )+.5)
        ...
        p[0,:] = quadratize.flatMatQuads(red_ar) + quadratize.flatMatQuads(np.fliplr(red_ar2)) #np.fliplr(r 
        p[1,:] = quadratize.flatMatQuads(gre_ar) + quadratize.flatMatQuads(np.fliplr(gre_ar2))
        p[2,:] = quadratize.flatMatQuads(blu_ar) + quadratize.flatMatQuads(np.fliplr(blu_ar2))
