...
from fn.whiterain import whiterain
cnt2 = np.array([0,0,0])
...
class umbrella5:
    def umbrella5(y):
            ...
            cnt5+=1
            p = .5*whiterain.whiterain(y)
            for i in range(0,len(p[0,:])):
                if i in umbrella:
                    p[:,i] = 0
            for i in range(0,len(umbrella)):
                if i%2 == 1:  
                    p[0,umbrella[i]] = 150*(.5*np.sin(cnt5/5+np.pi/6)+.5)
                    p[1,umbrella[i]] = 0
                    p[2,umbrella[i]] = 200*(.5*np.sin(cnt5/5)+.5)
                else:
                    p[0,umbrella[i]] = 200*(.5*np.sin(cnt5/5+np.pi)+.5)
                    p[1,umbrella[i]] = 0
                    p[2,umbrella[i]] = 150*(.5*np.sin(cnt5/5+np.pi+np.pi/6)+.5)
            return p
