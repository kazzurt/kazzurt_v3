...
class umbrella6:
    def umbrella6(y):
            ...
            p[1,:] = 0
            p[:,umbrella] = 255
            sz = int((90*np.sin(cnt5/5)+100))
            ran = np.random.randint(0,len(p[0,:]),size=sz)
            for i in range(0,len(ran)):
                if ran[i] in umbrella:
                    p[:,ran[i]] = 0
                    p[rn.randint(0,3),ran[i]] = 255
#             for i in range(0,len(umbrella)):
#                 if i%2 == 1:  
#                     p[0,umbrella[i]] = 255*(.4*np.sin(cnt5/10+np.pi/6)+.6)
#                     p[1,umbrella[i]] = 255*(.4*np.sin(cnt5/10+np.pi/3)+.6)
#                     p[2,umbrella[i]] = 255*(.4*np.sin(cnt5/10)+.6)
#                 else:
#                     p[0,umbrella[i]] = 255*(.4*np.sin(cnt5/10+np.pi/2)+.6)
#                     p[1,umbrella[i]] = 255*(.4*np.sin(cnt5/10+np.pi/2+np.pi/3)+.6)
#                     p[2,umbrella[i]] = 255*(.4*np.sin(cnt5/10+np.pi/2+np.pi/6)+.6)
