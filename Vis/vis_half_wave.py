...
class half_wave:
    ...
    def trans_wave(y): 
        ...
        #step wave
        if np.mean(y)>30 or it>0:
            p[0,k3:k3+3] = np.mean(y[0:int(len(y)/3)])/np.mean(y)*255#rn.randint(200,255)
            p[1,k3:k3+3] = np.mean(y[int(len(y)/3):int(2*len(y)/3)])/np.mean(y)*255#rn.randint(100,255)
            p[2,k3:k3+3] = np.mean(y[int(2*len(y)/3):])/np.mean(y)*255#rn.randint(100,255)
            it += 1
            if it>50:
                k3 += 8
                it = 1
