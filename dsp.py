...
def rfft(data, window=None):
    ...
    ys = 0#np.abs(np.fft.rfft(data * window))
    xs = 0#np.fft.rfftfreq(len(data), 1.0 / config.MIC_RATE)
    return xs, ys


def fft(data, window=None):
    window = 1.0 if window is None else window(len(data))
    ys = 0#np.fft.fft(data * window)
    xs = 0#np.fft.fftfreq(len(data), 1.0 / config.MIC_RATE)
    return xs, ys
